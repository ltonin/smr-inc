function [coeff, support] = smrinc_get_forceprofile(inclim, nrpt, bias, degree)
% SMRINC_GET_FORCEPROFILE Get force profile
%   coeff = SMRINC_GET_FORCEPROFILE(inclim, nrpt) returns the force profile
...
%
%   coeff = SMRINC_GET_FORCEPROFILE(inclim, nrpt, bias) add a bias to one
%   direction (towards 0 or towards 1). By default, the profile is balanced
%   (bias=0.5).
%
%   coeff = SMRINC_GET_FORCEPROFILE(inclim, nrpt, bias, degree) specify the
%   degree of the polynomial fit (by default, degree=8)
%
%   [coeff, support] = SMRINC_GET_FORCEPROFILE(...) returns also a
